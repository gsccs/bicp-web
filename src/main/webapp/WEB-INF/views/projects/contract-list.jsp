...
<script type="text/javascript">
	...
	$(function() {
		$("#list_data").datagrid({
			...
			fit : true,//自动大小 
			loadMsg : '数据加载中请稍后……',
			url : basepath+'/contract/datagrid',
			remoteSort : false,
			fitColums : true,
			checkOnSelect : true,
	...
	function addFun() {
		document.location = basepath+'/merchant/dataform';
	}
	...
	function editFun() {
		...
		if (checkedRows.length == 1) {
			document.location = basepath+'/contract/dataform?id='+checkedRows[0].ispcode;
	...
	function addPropFun() {
		var checkedRows = $("#list_data").datagrid('getChecked');
		if (checkedRows.length == 1) {
			var fm_window_dialog = $('#fm_window').dialog({
			title : '产品属性编辑',
			width : 600,
			height : 400,
			closed : false,
...
function editPropvFun() {
	...
	if (checkedRows.length == 1) {
		var propv_window_dialog = $('#fm_window').dialog({
			title : '产品属性值编辑',
			width : 600,
			height : 400,
			closed : false,
